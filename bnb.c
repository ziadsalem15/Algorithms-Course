...
void frac_bound(struc_sol *sol, int fix)
{
  ...
  do
    ...
    // update total weight and total value by adding the current one
      totalw += item_weights[temp_indexes[i]];
      totalp += item_values[temp_indexes[i]];
      i++;
...
void branch_and_bound(int *final_sol)
{
  ...
  /* YOUR CODE GOES HERE */
  // allocate memory for pqueue
  pqueue = (struc_sol *)malloc(sizeof(struc_sol)*SIZE);
  ...
  // LOOP until queue is empty or upper bound is not greater than current_best:
  while ((QueueSize > 0) || (pqueue->bound <= current_best))
  {
    // remove the first item in the queue
    sol = removeMax();
    // increase fixed by 1 and check if it is still less than N items
    int fixed = sol.fixed + 1;
    if (fixed > Nitems)
    {
      return;
    }
    //   construct two children, 1 with a 1 added, 1 with a O added
    struc_sol firstChild;
    // copy the current sol array to first child one
    copy_array(sol.solution_vec, firstChild.solution_vec);
    for (int i = 1; i < fixed; i++)
    {
      firstChild.solution_vec[i] = sol.solution_vec[i];
    }
    // set first child to 0 at the fixed index
    firstChild.solution_vec[fixed] = 0;
    firstChild.fixed = fixed;
    // compute its value and bound
    frac_bound(&firstChild, firstChild.fixed);
    // update current best if value is bigger
    if (firstChild.val > current_best)
    {
      current_best = firstChild.val;
      // for the rest of the indexs set them to 0
      for (int i = firstChild.fixed + 1; i <= Nitems; i++)
      {
        firstChild.solution_vec[i] = 0;
      }
      // copy array to final sol
      copy_array(firstChild.solution_vec, final_sol);
    }
    //       add child to the queue
    ...
    struc_sol secChild;
    copy_array(sol.solution_vec, secChild.solution_vec);
    // set at index fixed second child to one
    secChild.solution_vec[fixed] = 1;
    secChild.fixed = fixed;
    // calc its value an dbound
    frac_bound(&secChild, secChild.fixed);
    // update current best if value is bigger
    if (secChild.val > current_best)
    {
      current_best = secChild.val;
      // set rest of indexes to 0
      for (int i = secChild.fixed + 1; i <= Nitems; i++)
      {
        secChild.solution_vec[i] = 0;
      }
     //copy child to final_sol
      copy_array(secChild.solution_vec, final_sol);
    }
    // add child to queue 
    if(secChild.val != -1)
    {
