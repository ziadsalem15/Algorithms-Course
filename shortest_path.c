...
void print_path(FILE *f, path_t const *p) {
  if (p == NULL) fprintf(f,"NULL");
  else if (path_len(p) == 0) fprintf(f,"EMP");
  else {
    fprintf(f,"%zu",path_get(p,0));
    for (size_t i=1;i<path_len(p);++i)
      fprintf(f,", %zu",path_get(p,i));

  }
}

void print_sp_result(FILE *f, sp_result_t const *r) {
  fprintf(f,"Distance: "); print_weight(f,r->dist); fprintf(f,"\n");
  fprintf(f,"Path: "); print_path(f,r->path); fprintf(f,"\n");
  fprintf(f,"# Relaxed nodes: %llu\n", r->relax_count);
}
